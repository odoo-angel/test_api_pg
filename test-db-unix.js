...
// Detectar si es conexión Unix Socket o TCP (Cloud SQL proxy / Cloud Run)
const isUnixSocket =
...
if (isUnixSocket) {
  // 🔹 Unix Socket (Cloud SQL Auth Proxy, Cloud Run, App Engine, etc.)
  // Ejemplo: /cloudsql/proyecto:region:instancia
  poolConfig.host = process.env.DB_HOST;
  // NO SSL aquí: el proxy ya cifra el tráfico entre proxy ↔ instancia
  console.log('🔌 Modo: Unix Socket');
} else {
  // 🔹 TCP (IP pública/privada directamente a la instancia)
  ...
  if (process.env.DB_SSL === 'true') {
    ...
    poolConfig.ssl = {
      ...
      ca: fs.readFileSync(
        path.join(__dirname, 'config', 'ssl', 'server-ca.pem'),
        'utf8'
      ),
      key: fs.readFileSync(
        path.join(__dirname, 'config', 'ssl', 'client-key.pem'),
        'utf8'
      ),
      cert: fs.readFileSync(
        path.join(__dirname, 'config', 'ssl', 'client-cert.pem'),
        'utf8'
      ),
      // Solo aplica cuando vas por IP/hostname y quieres validar el cert
      servername:
        '1-773b0a9c-920b-40a5-846c-03f6f58d9910.us-central1.sql.goog',
    };
...
console.log('-----------------------------------');
console.log(`Host (Unix Socket/TCP): ${poolConfig.host}`);
console.log(`Database: ${poolConfig.database}`);
console.log(`User: ${poolConfig.user}`);
console.log(
  `Password: ${poolConfig.password ? '***' + poolConfig.password.slice(-3) : 'Not set'}`
);
console.log('-----------------------------------\n');
...
async function testConnection() {
  ...
  try {
    console.log(
      `🔌 Intentando conectar via ${
        isUnixSocket ? 'Unix Socket' : 'TCP/IP'
      }...\n`
    );
    ...
    for (const table of mainTables) {
      try {
        ...
        console.log(
          `  ${table}: Tabla no existe o error (${err.message})`
        );
      }
    }
    console.log('');
    ...
    try {
      await client.query(
        'CREATE TEMP TABLE test_write (id SERIAL, message TEXT)'
      );
      await client.query(
        "INSERT INTO test_write (message) VALUES ('Test Unix Socket/TCP')"
      );
    ...
    console.log(
      `  Server IP: ${
        connInfoResult.rows[0].server_ip || 'Unix Socket (no IP)'
      }`
    );
    console.log(
      `  Server Port: ${
        connInfoResult.rows[0].server_port || 'Unix Socket (no port)'
      }`
    );
    console.log('');
    ...
    console.log('\n💡 Troubleshooting:');
    console.log(
      '1. Verifica que el Cloud SQL Proxy esté corriendo (si usas Unix Socket)'
    );
    console.log(
      '2. Verifica que la ruta del socket o el host/IP sean correctos'
    );
    console.log(
      '3. Verifica que el servicio tenga permisos para acceder a la instancia'
    );
    console.log('4. Verifica las credenciales en el archivo .env');
    console.log(
      '5. Para Cloud Run, asegúrate que la instancia esté conectada en el deployment'
    );
  } finally {
...
// Ejecutar el test
console.log('🚀 Iniciando prueba de conexión PostgreSQL\n');
testConnection();
