...
if (isUnixSocket) {
  ...
  if (process.env.DB_SSL === 'true') {
    ...
    poolConfig.ssl = {
      ...
      // IMPORTANTE: debe coincidir con el DNS que salió en el error del certificado
      servername: '1-773b0a9c-920b-40a5-846c-03f6f58d9910.us-central1.sql.goog',
    };
...
async function testConnection() {
  ...
  try {
    console.log(`🔌 Intentando conectar via ${isUnixSocket ? 'Unix Socket' : 'TCP/IP'}...\n`);
    ...
    try {
      ...
      await client.query("INSERT INTO test_write (message) VALUES ('Test Unix Socket/TCP')");
    ...
    console.log('\n💡 Troubleshooting:');
    console.log('1. Verifica que el Cloud SQL Proxy esté corriendo (si usas Unix Socket)');
    console.log('2. Verifica que la ruta del socket o el host/IP sean correctos');
    console.log('3. Verifica que el servicio tenga permisos para acceder al socket o a la instancia');
